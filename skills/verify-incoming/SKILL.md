...
# Verify-Incoming Skill — SPECTRA (blocking, symmetric)

Receiver-side integrity gate for inbound ECL hand-offs. When an upstream
artefact arrives with a sibling `.envelope.json`, SPECTRA MUST NOT process the
...
SPECTRA's tool surface cannot run the SHA-256 gate itself (receiver Eidolons
have restricted or no Bash). The mechanical check therefore runs **at the
orchestrator**, once, before SPECTRA is dispatched:
...
`message_id`. SPECTRA then enforces the result using **only `Read`** — no Bash
required:
...
**Defense-in-depth (optional):** if SPECTRA's host happens to grant Bash with
...
## Inbound edges SPECTRA accepts

`to.eidolon` MUST equal `spectra`. `from.eidolon` MUST be a declared upstream
sender. Performative and `artifact.kind` are validated against the ECL inbound
contract `contracts/<from>-to-spectra.yaml` (source of truth); the table below
...
2. Print to stderr: `[spectra-verify-incoming] REFUSE: <FAILURE_CODE> from <from.eidolon>`.
...
**verify_pass:**
```json
{"ts":"<RFC3339>","event":"verify_pass","message_id":"<uuid>","thread_id":"<uuid>","from":"<eidolon>@<version>","to":"spectra@<version>","performative":"<performative>","integrity_method":"sha256"}
```

**verify_fail:**
```json
{"ts":"<RFC3339>","event":"verify_fail","message_id":"<uuid>","thread_id":"<uuid>","from":"<eidolon>@<version>","to":"spectra@<version>","integrity_method":"sha256","verify_failure_code":"<CODE>","decision":"refused"}
```
